...
<body>
    ...
    <form action="{{route('update', $student['id'])}}" method="POST">
        ...
        <div style="display: flex; margin-bottom: 15px">
            ...
            <input type="text" name="nama" id="nama" value="{{$student['nama']}}" placeholder="Masukan Nama..">
        </div>
        <div style="display: flex; margin-bottom: 15px">
            <label for="nis">NIS</label>
            <input type="number" name="nis" id="nis" value="{{$student['nis']}}" placeholder="Masukan Nis..">
        </div>
        <div style="display:flex; margin-bottom:15px">
            <label for="nis">Nis</label>
            <select name="rombel" id="rombel" >
                ...
                <option v<!DOCTYPE html>
                    ...
                    </html>alue="PPLG X" {{$student['rombel']  == 'PPLG X' ? 'selected' : ''}}>PPLG X</option>
                <option value="PPLG XI" {{$student['rombel']  == 'PPLG XI' ? 'selected' : ''}}>PPLG XI</option>
                <option value="PPLG XII" {{$student['rombel']  == 'PPLG XII' ? 'selected' : ''}}>PPLG XII</option>
            </select>
        </div>
        <div style="display: flex; margin-bottom: 15px">
            <label for="rayon">Rayon</label>
            <input type="text" name="rayon" id="rayon" value="{{$student['rayon']}}" placeholder="Rayon anda..">
        </div>
        <button type="submit">Kirim</button>
    </form>
</body>
